	...
	bool possibleToStamp(std::vector<std::vector<int>>& grid, int stampHeight,
		...
		for (std::size_t r = 1; r <= R; r++) {
			for (std::size_t c = 1; c <= C; c++) {
				psa[r][c] = psa[r - 1][c] + psa[r][c - 1] - psa[r - 1][c - 1] +
					grid[r - 1][c - 1];
			}
		}

		auto taken = [&psa](std::size_t tr, std::size_t tc, std::size_t br,
											std::size_t bc) {
				const auto sum = psa[br][bc] + psa[tr - 1][tc - 1] -
					psa[tr - 1][bc] - psa[br][tc - 1];
		...
		for (std::size_t r = 1; r <= R - SH + 1; r++) {
			...
			if (!taken(r, 1, r + SH - 1, C)) {
				...
				for (std::size_t rm = minR - 1; rm < r - 1 + SH;
				rm++) {
