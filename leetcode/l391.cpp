...
void pushDown(std::vector<Node>& nodes, const std::size_t idx) {
	auto& root = nodes[idx];
	if (root.dirty) {
...
bool update(std::vector<Node>& nodes, const std::size_t idx,
		const std::size_t tl, const std::size_t tr, const std::size_t l,
		const std::size_t r, const int expected, const int newVal) {
	...
	if (l == tl && r == tr) {
		...
		if (l <= tm) {
			if (!update(nodes, idx * 2, tl, tm, l, std::min(r, tm), expected,
						newVal))
				return false;
		}

		if ((tm + 1) <= r) {
			if (!update(nodes, idx * 2 + 1, tm + 1, tr, std::max(l, tm + 1), r,
						expected, newVal))
				return false;
		}
...
void printOut(std::vector<Node>& nodes, const std::size_t idx,
		const std::size_t tl, const std::size_t tr, int level = 0) {
	...
	std::cout << idx << "[" << tl << ", " << tr << "]: even=" << root.even
		<< ", y=" << root.y << std::endl;
